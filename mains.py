...
df = pd.read_excel ("C:\\Users\\DIPEN HORE\\Downloads\\Compressed\Material_Session 04\\Flask Deployment\\EPS_Dataset.xlsx",'v8')
...
# ... your code for prediction and evaluation ... 

# Save the model directly using XGBoost's save_model
model.save_model('eps_v1.json')  # Save as an XGBoost model file

# Load the model using XGBoost's load_model
load_model = xgb.Booster()  # Create an XGBoost Booster object
load_model.load_model('eps_v1.json')
 

arg = x_train[:2]  # Sample data from your XGBoost model

# Convert NumPy array to DMatrix
dmatrix_arg = xgb.DMatrix(arg)

# Predict using the DMatrix object
predictions = load_model.predict(dmatrix_arg) 
